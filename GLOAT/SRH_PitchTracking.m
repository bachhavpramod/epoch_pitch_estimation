function [f0,VUVDecisions,SRHVal] = SRH_PitchTracking(wave,framesize,shift,Fs,F0min,F0max)
...
% function [res] = GetLPCresidual(wave,frame length, shift,order,gci,type,t0)
res = GetLPCresidual(wave,framesize,shift,LPCorder);

%% Estimate the pitch track in 2 iterations
% PrevF0meanEst=0;
for Iter=1:Niter   

    [f0,SRHVal] = SRH_EstimatePitch(res',framesize,shift, Fs,F0min,F0max);
...
function [f0,SRHVal] = SRH_EstimatePitch(sig,framesize,shift,Fs,F0min,F0max)    % by me
stop=framesize;
