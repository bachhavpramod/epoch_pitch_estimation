...
% I made interp=0 and extrap=0 so as to avoid interpolation of unvoiced regions and insertion of values at start and end of speech file respectively 
...
% 
% if (fid == -1)
%     warning('ptch_fix.m -> Warning: cannot open %s! Using defaults...\n', CmdFile);
% else
%     while feof(fid) ~= 1 
%         s = upper(fgetl(fid));
%         [varname, s] = strtok(s);
